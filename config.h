...
static char *font2[] = {"JetbrainsMono Nerd Font:size=12"};
...
# if defined (XURLS) || defined (ESCAPER)
#     error (conflicting macro names)
# endif

/* url matcher */
// # define XURLS "tr ' ' '\\n' | perl -ne \"/(\\b([a-zA-Z]+:\\/\\/\\w[\\w-.:@]*|www\\.\\w[\\w-]*\\.\\w[\\w-.]*)(\\/[^\\\"' ()]*)?)/ && print\\\"\\$1\\n\\\"\" | tac | awk '!visited[$0]++'"
# define XURLS "tr ' ' '\\n' | urifind | tac | awk '!visited[$0]++'"
/* escaper */
# define ESCAPER "perl -pe s/\\(\\[\\!\\#\\\\\\$\\%\\&\\\\\\*\\\\\\(\\\\\\)\\ \\=\\'\\\"\\\\\\\\\\\\\\|\\\\\\[\\\\\\]\\`\\~\\,\\<\\>\\\\\\?\\]\\)/\\\\\\\\\\\\1/g;"
/* open urls */
...
void paste_esc ()
{
	FILE * clip_escaped = popen ("xclip -o -selection clipboard | "ESCAPER, "r");
